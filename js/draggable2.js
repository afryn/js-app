    ...
    $(dragAndDrop)
      ...
      .each(function (index, el) {
        $.each(dropdownText, function (i, dropdownTextVal) {
          if (index == i) {
            $(el).append(
              '<option value="correct">' + dropdownTextVal + "</option>"
            );
          } else {
            $(el).append(
    ...
    function checkDropdown() {
      ...
      if (numberOfChecks == 3) {
        ...
        if ($(dropdowns).css("visibility") == "visible") {
          dragAndDrop
            .find(".wjec-drag-and-drop-text")
            .css({ "max-height": "150px", "overflow-y": "scroll" });
        }
      }
  ...
  $.fn.dragAndDropWords = function (options) {
    ...
    function initialDragAndDropSetUp() {
      $.each(drops, function (index, element) {
        ...
        $(element).data("correct-answer", dropText).text("");

        dragsHolder.append(
          '<span class="wjec-drag">' +
            dropText +
            '<i class="fa fa-times"></i><i class="fa fa-check"></i></span>'
        );
      });
    ...
    function handleDropEvent(event, ui) {
      ...
      if (oldDrag.length > 0) {
        oldDrag.appendTo(thisDragParent).css({
          left: 0,
          top: 0,
        });
